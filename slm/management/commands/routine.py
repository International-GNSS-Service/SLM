...
def order_routines(routines):
    ...
    return [
        (routine[0], routine[1], routine[2] if len(routine) > 2 else None) for routine in
        sorted(routine_list, key=lambda r: r[0])
    ]
...
class Command(BaseCommand):
    ...
    ROUTINES = {
        routine: order_routines(groups) for routine, groups in
        getattr(
            settings,
            'SLM_ROUTINES',
            {}
    ...
    def add_arguments(self, parser):
        ...
        routine_parsers = parser.add_subparsers(
            help='Invoke specific batch routine.'
        )
        ...
        for routine, groups in routines.items():
            r_parser = routine_parsers.add_parser(
                routine,
                help=getattr(settings, f'SLM_ROUTINE_{routine.upper()}', '')
            )
            ...
            if phases:
                for phase, subroutines in phases.items():
                    ...
                    phase_help = getattr(
                        settings,
                        f'SLM_ROUTINE_{routine.upper()}_{phase.upper()}',
                        ''
                    )
                    if phase_help:
