...
class Command(TyperCommand):
                    ...
                    for member in archive.getmembers():
                        ...
                        try:
                            ...
                            if prep_date:
                                ...
                                if (
                                    prep_date.month == log_time.month
                                    and prep_date.year == log_time.year
                                    and no_day
                                ):
                                    log_time = make_aware(
                                        datetime(
                            ...
                            if (
                                log_date
                                and site.join_date is None
                                or site.join_date > log_time.date()
                            ):
                                site.join_date = log_time.date()
                                site.save()

                            index = ArchiveIndex.objects.insert_index(
                                site=site, begin=log_time
                            )
    ...
    def log_params(self, log_str, site):
        bound_log = SiteLogBinder(
            SiteLogParser(self.decode_str(log_str), site_name=site.name)
        ).parsed

        def get_param(section_index, field_name, null_val=None):
            binding = getattr(
                getattr(bound_log, "sections", {}).get(section_index, {}), "binding", {}
            )
            if binding:
        ...
        for index, section in bound_log.sections.items():
            if index[0] == 3 and section.contains_values and section.binding:
                ...
                params["serial_number"] = section.binding.get("serial_number", "")
